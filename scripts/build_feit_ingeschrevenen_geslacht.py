"""
build_feit_ingeschrevenen_geslacht.py
Bouwt feit_ingeschrevenen_geslacht vanuit p01a (ingeschrevenengeslhbo.csv).

Korrel: studiejaar × gemeente × instelling × sector × geslacht
Meetwaarde: aantal_ingeschrevenen
Privacyonderdrukking: -1 → NULL + is_onderdrukt = True

Let op: geen CROHO-code — opleiding is op sectorniveau (dim_sector).

Vereisten (run eerst):
  build_dim_tijd.py, build_dim_instelling.py,
  build_dim_sector.py, build_dim_geografie.py, build_dim_geslacht.py
"""
import sys
...
print("Bouwen feit_ingeschrevenen_geslacht...")

# --- Brondata laden ---
df = read_stat_csv(RAW / "p01hoinges" / "ingeschrevenengeslhbo.csv")
...
write_parquet(df_feit, "feit_ingeschrevenen_geslacht")
