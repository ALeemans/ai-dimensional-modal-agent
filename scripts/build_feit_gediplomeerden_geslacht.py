"""
build_feit_gediplomeerden_geslacht.py
Bouwt feit_gediplomeerden_geslacht vanuit p04a (gediplomeerdengeslhbo.csv).

Korrel: diplomajaar × gemeente × instelling × sector × soort_diploma × geslacht
Meetwaarde: aantal_gediplomeerden

Let op: p04a heeft SOORT_DIPLOMA i.p.v. TYPE_HOGER_ONDERWIJS.
...
Vereisten (run eerst):
  ...
  build_dim_geografie.py, build_dim_soort_diploma.py, build_dim_geslacht.py
"""
import sys
...
print("Bouwen feit_gediplomeerden_geslacht...")

df = read_stat_csv(RAW / "p04hogdipl" / "gediplomeerdengeslhbo.csv")
